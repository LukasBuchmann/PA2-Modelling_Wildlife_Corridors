...
#| tbl-cap: "List of Top Critical Bottlenecks identified by the model. IDs correspond to the labels in Figure 3. Coordinates are in CH1903+/LV95."
#| echo: false

import pandas as pd
from IPython.display import Markdown

# CORRECTED PATH: Use '../' to go up from the Report folder to the Project Root
csv_path = "../results/bottlenecks_table.csv"

try:
    df = pd.read_csv(csv_path)
    
    # 2. Format the table
    # Combine coordinates if columns exist, otherwise handle gracefully
    if 'Coords_E' in df.columns and 'Coords_N' in df.columns:
        df['Coordinates'] = df['Coords_E'].astype(str) + " / " + df['Coords_N'].astype(str)
    
    # Select columns (Adjust 'ID' or 'Path_Intensity' if your CSV headers differ)
    # We use a safe selection to avoid KeyErrors
    cols_to_show = ['ID', 'Coordinates', 'Path_Intensity']
    df_display = df[cols_to_show].copy()
    
    # Rename for the report
    df_display.columns = ['Map ID', 'Coordinates (LV95)', 'Path Intensity']

    # 3. Render
    # Note: Requires 'tabulate' installed (pip install tabulate)
    Markdown(df_display.to_markdown(index=False, numalign="center", stralign="center"))

except FileNotFoundError:
    print(f"Error: Could not find file at {csv_path}. Did you run the analysis script?")
