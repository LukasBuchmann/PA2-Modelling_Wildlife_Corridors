...
The developed resistance surface (Figure 1) represents the landscape permeability for Capreolus capreolus in the Canton of Schaffhausen. Resistance values range from 1 (optimal habitat) to 5000 (barriers) on a logarithmic scale. The spatial analysis reveals a highly fragmented landscape. Urban centers, particularly the city of Schaffhausen, in the middle of the image, the city of Singen in the north east of the image and the Rhine river corridor, which flows from east to west, form distinct high-resistance barriers (Red areas in Fig. 1). In addition, the A4 motorway bisects the study area to the south, creating a continuous high-resistance feature (Cost = 5000). Conversely, the Randen massif and the Klettgau agricultural zones in the north west form large, contiguous patches of low resistance (Green and Yellow area in Fig. 1). In general, the landscape is characterized by a patchwork of forested areas (Resistance = 1–5) interspersed with agricultural land (Resistance = 20–50) and fragmented by linear infrastructure elements (Resistance = 200–5000). 

![Resistance Surface](../results/map_01_resistance_surface.png)


## The Modeled Connectivity Network
The LCP analysis identified 53 core habitat nodes suitable for Roe Deer. The resulting connectivity network (Figure 2) reveals the cumulative traffic intensity of all modeled paths.
...
By filtering the top 5% of traffic intensity combined with high resistance values (barriers), specific bottlenecks were identified (Figure 3, Blue Circles). The analysis highlights a concentration of bottlenecks along the main transport axes (e.g., A4 highway) and the railway lines bordering the Rhine.

Spatial Distribution: Bottlenecks are not evenly distributed, they cluster significantly in the transition zones between the agricultural plateau and the urbanized valley floors. 

Validation with Traffic Incident Data: An overlay of the identified bottlenecks with the provided deer traffic incident data ($N=...$) shows that $XX\%$ of the modeled high-risk crossings are located within a $500$m buffer of recorded wildlife-vehicle collisions (see Figure 4 [You need to create this]).



## Additional Findings
Kernel Density Adjustment: To account for spatial divergence in LCPs, a focal sum analysis revealed that the effective width of functional corridors varies between $X$ and $Y$ meters.


# Discussion

## Interpretation of Connectivity Patterns
The results confirm that while the Canton of Schaffhausen retains significant core habitats, the functional connectivity for Capreolus capreolus is severely compromised by linear infrastructure. The concentration of paths (Figure 2) suggests that roe deer are forced into "funnel" systems. This aligns with findings by [Author, Year] regarding the "barrier effect" of Swiss roadways. The identified bottlenecks represent areas where the "permeability" of the landscape is critical; the loss of these specific corridors could lead to the genetic isolation of the subpopulations in the [Region Name].

## Model Validation and Real-World Relevance
The validity of the resistance surface is supported by the comparison with deer traffic incident data. The strong spatial correlation ($XX\%$) suggests that the cost weights assigned to "Roads" and "Urban edges" accurately reflect the actual movement risks perceived by the animals.
Notably, the model identified a high-priority bottleneck at coordinates [Insert Coordinates]. This location corresponds to an existing wildlife underpass/bridge. This "ground truth" discovery demonstrates the model's capability to correctly identify optimal crossing locations based solely on topographic and land-cover data.

## Methodological Reflection and Limitations
A challenge encountered during the LCP analysis was the "path smearing" effect, where mathematically distinct paths crossed linear barriers in close proximity but not identical pixels. This resulted in an initial underestimation of bottleneck intensity in Figure 2.

- Resolution: The use of [10m/25m] resolution raster data may oversimplify micro-topographic features (e.g., steep embankments) that act as local barriers.
- Static Nature: The model assumes a static landscape and does not account for temporal variations such as seasonal crop heights or traffic volume fluctuations, which are known to influence deer behavior.

## Implications for Spatial Planning
The identified "High-Risk Crossings" (Figure 3) should be prioritized for mitigation measures. Specifically, the corridor at [Location Name] shows high movement pressure but high resistance, making it an ideal candidate for a green bridge or fencing improvements.
